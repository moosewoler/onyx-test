...
static int test_animation_mode(void)
{
	...
	copy_image_to_buffer(0, 0, 600, 600, pic_octcat_bmp, BUFFER_FB, &screen_info);
	printf("Octcat update normal\n");
	update_to_display(0, 0, 800, 600, WAVEFORM_MODE_AUTO, TRUE, 0);
	sleep(2);
	...
	printf("Octcat update black/white\n");
	...
	printf("Blank screen\n");
	memset(fb, 0xFF, screen_info.xres_virtual*screen_info.yres*screen_info.bits_per_pixel/8);
	update_to_display(0, 0, screen_info.xres, screen_info.yres, WAVEFORM_MODE_GC16, TRUE, 0);
